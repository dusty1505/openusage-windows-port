...
Use this when UsageForge is not working and you need to share debug info.
...
1. In your macOS menu bar, find the UsageForge icon.
2. Right-click it (or hold `Control` and click).
3. Open `Debug Level`.
4. Select `Debug`.

If UsageForge does not open at all, skip this step and continue.
...
```text
~/Library/Logs/com.dusty1505.usageforge
```
...
1. Attach `usageforge.log`.
2. If you also see files like `usageforge.log.1`, attach those too.
