...
1. **UsageForge Keychain** (`UsageForge-copilot`) — Token previously cached by the plugin
...
Once authenticated via gh CLI, the plugin caches the token in the UsageForge keychain for faster access on subsequent probes.
